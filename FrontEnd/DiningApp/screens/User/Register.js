...
const Register = () => {
    ...
    const register = async () => {
        ...
        try {
            ...
            if (user.avatar) {
                ...
                data.append("file", { uri: user.avatar.uri, type: user.avatar.mimeType || "image/jpeg", name: "upload.jpg" });
                ...
                const res = await axios.post(`https://api.cloudinary.com/v1_1/${process.env.EXPO_PUBLIC_CLOUDINARY_CLOUD_NAME}/image/upload`, data,
    ...
    return (
        <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 24 }}>
            ...
            {fields.map(f =>
                <TextInput key={f.field} mode="outlined" label={f.title}
            ...
            <Button mode="contained" loading={loading} disabled={loading}
                onPress={register} style={{ marginTop: 8 }}>
                Đăng ký ngay
            </Button>
